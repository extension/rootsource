...
A static, no-build marketing website for RootSource, an agricultural-answers API/widget from Extension Foundation. There is no backend, no build tooling, no package manager, and no test suite — just HTML, one CSS file, and one small JS file.
